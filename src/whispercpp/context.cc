...
void ExportContextApi(py::module &m) {
  ...
  py::class_<Context>(m, "Context", "A light wrapper around whisper_context")
      ...
      .def("token_to_str", [](Context & context, whisper_token token_id) {
...
void new_segment_callback_handler(whisper_context * ctx, whisper_state *, int n_new, void * user_data) {
...
// defaults to 'en'.
void Params::set_language(const char *language) {
  wfp.language = language;
}
